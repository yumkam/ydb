...
Y_UNIT_TEST_SUITE(TestJsonParser) {
    ...
    Y_UNIT_TEST_F(MissingRepeatedFieldsValidation, TJsonParserFixture) {
        ...
        ParserHandler->ExpectColumnError(0, EStatusId::PRECONDITION_FAILED, TStringBuilder() << "Failed to parse json messages, found 1 missing values in non optional column 'a2' with type [DataType; String], buffered offsets: " << FIRST_OFFSET);
        ParserHandler->ExpectColumnError(1, EStatusId::PRECONDITION_FAILED, TStringBuilder() << "Failed to parse json messages, found 1 missing values in non optional column 'a1' with type [DataType; String], buffered offsets: " << FIRST_OFFSET + 1);
        ExpectedBatches++;
        Parser->ParseMessages({
    ...
    Y_UNIT_TEST_F(MissingRepeatedFieldsBufferOverflow, TJsonParserFixture) {
        ...
        ParserHandler->ExpectColumnError(1, EStatusId::PRECONDITION_FAILED, TStringBuilder() << "Failed to parse json messages, found 1 missing values in non optional column 'a2' with type [DataType; String], buffered offsets: " << FIRST_OFFSET);
        ExpectedBatches++;
