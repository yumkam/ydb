...
struct TFederationState {
    std::shared_ptr<NYdb::NFederatedTopic::TFederatedDbState> State;
    std::vector<NFq::TEvRowDispatcher::TEvStartSession::TPtr> PendingStartSessions;
};
...
struct TEvPrivate {
    ...
    struct TEvReceivedClusters : public NActors::TEventLocal<TEvReceivedClusters, EvReceivedDcClusters> {
        TEvReceivedClusters(TFederationState &store, std::shared_ptr<NYdb::NFederatedTopic::TFederatedDbState>&& state)
            : Store(store)
            , State(std::move(state))
        {}
        TFederationState& Store;
        std::shared_ptr<NYdb::NFederatedTopic::TFederatedDbState> State;
    };
};
    ...
    struct TFederationDescriptorHash {
        auto operator() (const TFederationDescriptor& desc) const {
            return CombineHashes<ui64>(
                    std::hash<std::string>()(desc.DiscoveryEndpoint),
                    std::hash<std::string>()(desc.Path)
            );
        }
    };

    std::unordered_map<TFederationDescriptor, TFederationState, TFederationDescriptorHash> DcClusterInfo;
    void StartClusterDiscovery(const TFederationDescriptor& federation, TFederationState &state);
...
public:
    ...
    void Handle(NFq::TEvPrivate::TEvReceivedClusters::TPtr&);
    ...
    STRICT_STFUNC(
        ...
        hFunc(NFq::TEvPrivate::TEvReceivedClusters, Handle);
...
void TRowDispatcher::Handle(NFq::TEvRowDispatcher::TEvStartSession::TPtr& ev) {
    ...
    for (auto partitionId : ev->Get()->Record.GetPartitionIds()) {
        ...
        TPartitionKey partitionKey { db->name(), partitionId };
