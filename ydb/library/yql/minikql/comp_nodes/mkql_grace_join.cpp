...
void DoCalculateWithSpilling(TComputationContext& ctx) {
    ...
    if (!HasMemoryForProcessing()) {
        ...
        YQL_LOG(INFO) << "yellow zone reached in Spilling mode " << (used*100/limit) << "%=" << used << "/" << limit << " isWaitingForReduce=" << isWaitingForReduce;
    ...
    if (!*HaveMoreLeftRows && !*HaveMoreRightRows) {
        if (!UpdateSpilling() || HasRunningAsyncOperation()) return;
