...
def SemiRandomSequence(messages):
    for xid in messages:
        rpt = 10
        r = random.randint(0, 5)
        if r < 1:
            r = random.randint(40, 10000000)
        else:
            r = random.randint(0, 40)
            rpt = 20
        yield (json.dumps({"id": xid, "user": str(r)*rpt, "payload": str(r)*1000}), )
    ...
    (
        R'''
            ...
            $enriched = select e.id as id,
                ...
                on(e.user = u.b)
            ;
            $enriched = SELECT * FROM $enriched WHERE uid IS DISTINCT FROM NULL or upayload IS DISTINCT FROM NULL;
        ...
        [
            (
                '{ "id":10000000, "user":"2", "payload":"abcdef"}',
                '{ "id":10000000, "user":"2", "payload":"abcdef", "uid": "2", "upayload": 3}'
            ),
            *SemiRandomSequence(range(2000000)),
            (
                '{ "id":10000001, "user":"8", "payload":"ghijkl"}',
                '{ "id":10000001, "user":"8", "payload":"ghijkl", "uid": "8", "upayload": 9}'
            ),
        ],
        "TTL",
        "10",
...
class TestJoinStreaming(TestYdsBase):
    ...
    @pytest.mark.parametrize("shared_reading", [True])
