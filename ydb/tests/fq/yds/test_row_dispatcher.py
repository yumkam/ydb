...
class TestPqRowDispatcher(TestYdsBase):
    ...
    def test_like_pushdown(self, kikimr, client, icase: bool, with_escape: bool):
        ...
        sql = Rf'''
            $contains = ($str, $const) -> {{
              RETURN $str {"I" if icase else ""}LIKE $const {' ESCAPE "!"' if with_escape else ""};
            }};
            INSERT INTO {YDS_CONNECTION}.`{self.output_topic}`
            SELECT event FROM {YDS_CONNECTION}.`{self.input_topic}`
                ...
                WHERE time > 100 AND time < 200 AND $contains(event, '%foobar%')
                -- event {"I" if icase else ""}LIKE "%foobar%"{' ESCAPE "!"' if with_escape else ""};'''
        ...
        # for now, check if partial pushdown is not broken
        assert with_escape or icase or "foobar" in issues
