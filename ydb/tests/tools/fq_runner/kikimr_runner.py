...
class BaseTenant(abc.ABC):
    ...
    def wait_worker_count(self, node_index, activity, expected_count, timeout=plain_or_under_sanitizer(30*2, 150)):
    ...
    def wait_bootstrap(self, node_index=None, wait_time=plain_or_under_sanitizer(90*2, 400)):
        if node_index is None:
            for n in self.kikimr_cluster.nodes:
    ...
    def wait_discovery(self, node_index=None, wait_time=plain_or_under_sanitizer(30*2, 150)):
        if node_index is None:
            for n in self.kikimr_cluster.nodes:
    ...
    def wait_workers(self, worker_count, wait_time=plain_or_under_sanitizer(30*2, 150)):
    ...
    def wait_completed_checkpoints(self, query_id, checkpoints_count,
                                   timeout=plain_or_under_sanitizer(30*2, 150),
